...
class FilterHacked extends Filter {
    cleanHacked(string) {
        try {
            ...
            const isOnlyEmoji = string.replace(/[\p{Emoji_Presentation}]/gu, '').trim().length === 0;
            if (isOnlyEmoji) {
            ...
            return string.split(this.splitRegex).map((word) => {
                const containsNonEmoji = word.replace(/[\p{Emoji_Presentation}]/gu, '').trim().length > 0;
                return containsNonEmoji && this.isProfane(word) ? this.replaceWord(word) : word;
            }).join(joinString);
        }
    }
}
